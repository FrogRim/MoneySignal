...
Validation failure, runtime failure, system failure는 success envelope를 절대 사용하지 않는다.
...
### 3. 운영 규칙은 `error.code` 중심으로 고정한다
- 클라이언트는 `503`만 보고 처리하지 않고 반드시 `error.code`로 세부 분기한다.
- `trace_id`는 API layer에서 생성하며 모든 structured error response에 포함한다.
...
- `trace_id`는 현재 error path에만 존재하며 success path 추적 계약은 아직 없다.
